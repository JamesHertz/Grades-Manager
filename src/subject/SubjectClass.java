...
public class SubjectClass implements Subject, Serializable {
    ...
    private final Map<Integer, SubjectSlot> allStudents;
    private final Set<String> evalIds; // is it really worth
    ...
    public EvalSheet addEvaluation(String evalId, SheetHelper helper) throws EvaluationAlreadyExistsException {
        ...
        while(it.hasNext()){
           ...
           if(slot == null){
            ...
            slot.addEvaluations(new EvalEntryClass(eval,st, tmp.grade()));
        }
        return eval;
    }
    ...
    public EvalSheet getEvalSheet(String evalId) throws EvaluationDoesNotExit {
        EvalSheet tmp = allEvaluations.get(evalId);
        if(tmp == null)
